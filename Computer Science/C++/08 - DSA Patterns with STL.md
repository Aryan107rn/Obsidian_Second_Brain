...
> [!tip] When to use
> counting occurrences, anagram checks, majority element.
...
> [!tip] When to use
> "next greater/smaller element" problems — O(n) instead of the naive O(n²).
...
> [!tip] When to use
> sliding window max/min in O(n) — the deque keeps candidates in monotonic order so the front is always the current window's max.
...
> [!tip] When to use
> connectivity queries, cycle detection in undirected graphs, Kruskal's MST.
