...
> [!tip] When to use
> default to `unique_ptr` unless multiple owners genuinely need to share the resource (`shared_ptr`). `weak_ptr` breaks cycles that would otherwise leak memory (e.g. two objects holding `shared_ptr`s to each other).
