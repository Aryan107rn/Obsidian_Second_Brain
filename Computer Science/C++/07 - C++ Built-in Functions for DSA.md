...
> [!tip] When to use
> subset enumeration (`for (int mask = 0; mask < (1<<n); mask++)`), bitmask DP (Travelling-Salesman-style states), checking power of 2 (`n && !(n & (n-1))`), XOR-based problems (single number, missing number).
...
> [!tip] When to use
> fixed-size bit arrays (sieve of Eratosthenes flags, visited sets over a bounded range) — more memory-efficient than `vector<bool>` and gives O(1) popcount via `.count()`.
