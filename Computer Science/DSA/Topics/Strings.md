...
> [!tip] When to apply
> checking symmetry, comparing from both ends, or in-place reversal.
...
> [!warning] Remember
> For "valid palindrome ignoring non-alphanumeric," skip invalid chars with inner while loops on both pointers before comparing.
...
> [!tip] When to apply
> "longest/shortest substring with [condition]" — no repeating characters, at most k distinct characters, contains all characters of another string, etc.
...
> [!example] Complexity
> O(n), Space: O(min(n, charset size))

> [!warning] Remember
> Jumping `l` directly to `lastSeen[s[r]] + 1` (instead of incrementing one at a time) is what keeps this O(n) instead of O(n²).
...
> [!tip] When to apply
> anagram checks, "group anagrams," character frequency comparisons.
...
> [!example] Complexity
> O(n), Space: O(26) = O(1) for lowercase-only alphabet.

> [!warning] Remember
> A sorted-string as a hashmap key (`sort(s.begin(), s.end())`) is the classic trick for "group anagrams" — anagrams sort to the same string.
...
> [!tip] When to apply
> find all occurrences of a pattern P in text T efficiently — naive substring search is O(n·m), KMP achieves O(n+m) by avoiding re-checking characters that are guaranteed to match.

> [!note] Intuition
> Precompute a "longest proper prefix that is also a suffix" (LPS) array for the pattern. On a mismatch, instead of restarting the pattern from index 0, jump using the LPS array — part of the text already matched part of the pattern, so redundant comparisons can be skipped.
...
> [!example] Complexity
> O(n + m), Space: O(m) for LPS array.

> [!warning] Remember
> The LPS array is the entire trick — "how much of the pattern's prefix can be reused" on a mismatch instead of restarting. This idea (failure function) reappears in the Z-algorithm and Aho-Corasick.
...
> [!tip] When to apply
> same use case as KMP (pattern matching, finding all occurrences), sometimes preferred for its simpler array meaning, and useful beyond matching too.

> [!note] Intuition
> Build a Z-array where `Z[i]` = length of the longest substring starting at i that matches a prefix of the string. To search for pattern P in text T, form `P + '#' + T` and look for positions where `Z[i] == len(P)`.

> [!example] Complexity
> O(n+m), Space: O(n+m)

> [!warning] Remember
> Z-array gives prefix-match lengths at every position directly — also useful for counting distinct substrings or finding the shortest repeating unit of a string.
...
> [!tip] When to apply
> pattern matching where average O(n+m) via hashing is preferred over KMP's failure function — generalizes well to multi-pattern search and 2D pattern matching.

> [!note] Intuition
> Compute a hash for the pattern and for each equal-length window of text. Compare hashes first (O(1) after rolling update); only do a full character comparison on a hash match (guards against collisions).
...
> [!warning] Remember
> Always verify with a direct character comparison when hashes match — silently trusting a hash match is a common bug source.
...
> [!tip] When to apply
> need fast prefix-based operations — autocomplete, "does any word start with prefix X," dictionary search, longest common prefix among many strings.

> [!note] Intuition
> Tree where each edge is a character; root-to-marked-node path spells a stored word. Shared prefixes share the same path, so lookups/prefix-checks cost O(word length), independent of how many words are stored.
...
> [!example] Complexity
> O(L) per operation (L = word/prefix length), Space: O(total characters across all inserted words) worst case.

> [!warning] Remember
> Trie beats a hashset of strings when **prefix** queries are needed — a hashset only supports exact-match, not "does anything start with this."
...
> [!tip] When to apply
> comparing two strings for shared structure — diff tools, DNA comparison, edit-distance-adjacent problems.
...
> [!warning] Remember
> Fundamentally a string-comparison pattern, but the full recurrence/code belongs in a dedicated DP note — flagged here for connection purposes.
...
> [!tip] When to apply
> compress consecutive repeated characters into count+char form, or decompress back — e.g. `"aaabbc"` → `"a3b2c1"`.
...
> [!warning] Remember
> Always compare compressed length to original — for strings with no repeats, "compression" makes the string longer.
...
> [!tip] When to apply
> group multiple strings by whether they're anagrams of each other.
...
> [!example] Complexity
> O(n·k log k), n = number of strings, k = max string length.

> [!warning] Remember
> A character-count tuple (26 ints) as key is faster (O(k) vs O(k log k) per string) than sorting, when the alphabet is small and fixed.
